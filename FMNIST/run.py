...
from TCSAttention import TCSAModule
import os
import time
import argparse
import numpy as np
...
class CupyNet(nn.Module):
    def __init__(self, T, r):
        ...
        self.conv = nn.Sequential(
            neuron.MultiStepLIFNode(tau=2.0,surrogate_function=surrogate.ATan(), backend='cupy'),
            layer.SeqToANNContainer(
                nn.MaxPool2d(2, 2),  # 14 * 14
                nn.Conv2d(128, 128, kernel_size=3, padding=1, bias=False),
                nn.BatchNorm2d(128),
            ),
            TCSAModule(T=self.T, H=14, W=14, C=128, rank=r, device='cuda:0'),
            neuron.MultiStepLIFNode(tau=2.0,surrogate_function=surrogate.ATan(), backend='cupy'),
            layer.SeqToANNContainer(
                nn.MaxPool2d(2, 2),  # 7 * 7
                nn.Flatten(),
...
def main():
    ...
    * :ref:`中文API <conv_fashion_mnist.main-cn>`
    ...
    parser.add_argument('-cupy', action='store_true', help='use cupy neuron and multi-step forward mode', default=True)
    ...
    if args.cupy:
        out_dir += '_cupy'
    out_dir+=f"_{args.r}_desc_{args.desc}"
